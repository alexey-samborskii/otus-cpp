...
void map_reduce()
{
    ...
    const auto reducer_output = std::accumulate(
        map_output.begin(),
        map_output.end(),
        0,
        reducer);
...
void map_shuffle_reduce()
{
    ...
    std::cout << "Number of unique by abs = "
              << reducer_output
              << std::endl;
}
...
std::string input(
    ...
    "running the various tasks in parallel, managing all communications "
...
std::string prepare(const std::string& input)
{
    ...
    std::copy_if(lower.begin(),
                 lower.end(),
                 std::back_inserter(alpha_lower),
                 [](unsigned char c) {
...
void almost_big_data()
{
    ...
    //  std::string - key (word)
    //  int         - value (count)
    std::vector<std::pair<std::string, int>> map_output;
    ...
    const auto last_word = std::accumulate(
        map_output.begin(),
        map_output.end(),
        std::pair<std::string, int>{"", 0},
        reducer);

    std::cout << last_word.first
              << " " << last_word.second
              << std::endl;
}
